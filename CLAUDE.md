...
- **AI path following**: Drones follow a **cyclic Catmull-Rom spline** (`CubicCurve<Vec3>` from Bevy's `CubicCardinalSpline::new_catmull_rom().to_curve_cyclic()`) through gate centers. `AIController` stores the spline and a continuous `spline_t` parameter (0 to gate_count). Progress is updated via local tangent projection in `update_ai_targets()`. `compute_racing_line()` samples the spline at a look-ahead offset for smooth curved flight. `RacePath` struct (in `spawning.rs`) bundles the spline with gate positions and gate forwards. Requires >= 2 gates.
- **Gate directionality**: Each gate has a canonical forward direction. `TriggerVolumeConfig.forward` defines the local-space forward (default `Vec3::NEG_Z`). Per-instance `ObstacleInstance.gate_forward_flipped: bool` flips it 180 degrees. World-space forward = `rotation * (if flipped { -forward } else { forward })`. `GateForward` component (on gate entities at race time) stores the computed world-space forward. `RacePath.gate_forwards` and `AIController.gate_forwards` carry per-gate forward vectors for AI and validation. In the course editor, gates with `gate_order` show a **cyan arrow gizmo** pointing in the forward direction. Press `F` with a gate selected to flip its direction.
