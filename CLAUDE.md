...
- **AI path following**: Drones follow a **cyclic Catmull-Rom spline** (`CubicCurve<Vec3>` from Bevy's `CubicCardinalSpline::new_catmull_rom().to_curve_cyclic()`) through gate centers. `AIController` stores the spline and a continuous `spline_t` parameter (0 to gate_count). Progress is updated via local tangent projection in `update_ai_targets()`. `compute_racing_line()` samples the spline at a look-ahead offset for smooth curved flight. `RacePath` struct (in `spawning.rs`) bundles the spline with gate positions. Requires >= 2 gates.
