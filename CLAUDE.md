...
- **Drone pattern**: `DroneGltfHandle` loaded `OnEnter(Race)`, `DroneAssets` extracted via `run_if(drone_gltf_ready)` + `run_if(not(resource_exists::<DroneAssets>))`, `spawn_drones` gated by `run_if(resource_exists::<DroneAssets>)` + `run_if(resource_exists::<CourseData>)`. 12 drones with `DespawnOnExit(AppState::Race)`. Physics: 11-system `.chain()` in FixedUpdate (thrust-through-body model). Each drone gets a unique spline via `generate_drone_race_path()`. Per-drone variation via `DroneConfig`. `RaceSeed` resource randomizes outcomes between races. Resources cleaned up `OnExit(Race)`.
