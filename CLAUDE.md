...
- **Drone pattern**: `DroneGltfHandle` is loaded `OnEnter(Race)`. `DroneAssets` is extracted from the glTF in an `Update` polling system (async pattern). `spawn_drones` polls until both `DroneAssets` and `CourseData` are available, then spawns 12 drones with `DespawnOnExit(AppState::Race)`. Resources are cleaned up `OnExit(Race)`. Physics runs in `FixedUpdate` as a 10-system `.chain()` using a thrust-through-body model: `hover_target → position_pid → attitude_controller → dirty_air_perturbation → motor_lag → apply_forces → integrate_motion → clamp_transform` (plus 2 AI systems when drones are active). Key components: `PositionPid` (outer loop, uses derivative-on-measurement via `-velocity`), `AttitudePd` (inner loop, slightly underdamped for visible settle wobble: kp=7.0, kd=0.20), `DesiredAttitude` (bridge between loops), `DroneDynamics` (velocity, angular velocity, thrust with motor lag 25ms, quadratic drag). Per-drone variation in `DroneConfig` includes PID gains, cornering aggression (±20%), braking distance (±20%), attitude PD multipliers (±10%), `racing_line_bias` (midleg lateral shift, correlated with aggression), `approach_offset_scale` (approach distance multiplier, inversely correlated with aggression), and `gate_pass_offset` (fraction 0.2–0.6 of each gate's trigger volume half-extents, used for 2D pass-through offset in both width and height — aggressive drones use wider offsets). Each drone generates its own unique spline via `generate_drone_race_path()` at spawn time. If the drone glb is missing, a red placeholder cube is used.
