...
- **Portrait pattern**: `portrait/` submodule with 4 files. `PortraitDescriptor` has 6 slot enums (FaceShape/EyeStyle/MouthStyle/HairStyle/ShirtStyle/Accessory) + color fields (including `shirt_color` derived via `derive_shirt_color()`). `PortraitDescriptor::generate(rng, primary_color)` creates randomized portraits. Accessory has 4 variants: Necklace, SpikedCollar, Piercings, Earring (with serde aliases for old names). `fragments.rs` contains hand-drawn Inkscape SVG fragments for a `"9.5 11.5 20.1 20.1"` viewBox with color placeholder tokens including `SHIRT_COLOR`. Portrait-03 fragments use `translate(-28,0)` wrapper. Layer order (back to front): bg, hair_back, face, shirt, eyes, mouth, hair_front, accessory. `assemble_svg()` selects fragments and replaces placeholders; `shirt_fragment()` selects shirt SVG. Fallback mappings collapse old enum variants to new ones (e.g., Long->Oval, Goggles->Wide, Helmet->Beanie). `rasterize.rs` uses `resvg` 0.47 to render SVG -> `tiny_skia::Pixmap` -> Bevy `Image` (48x48). `PortraitCache` resource (`HashMap<PilotId, Handle<Image>>`) built `OnEnter(Race)` via chained `setup_portrait_cache` system (after `select_pilots_for_race`), persists across races. Roster migration: `backfill_empty_portraits()` auto-generates portraits for Phase 1 pilots using deterministic RNG seeded by `PilotId`. Portraits displayed in leaderboard (`LbPortrait` component, 16x16) and results screen (20x20 `ImageNode`), with solid-color fallback.
