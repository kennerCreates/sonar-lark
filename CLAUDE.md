...
- **Workshop pattern**: `WorkshopState` created `OnEnter(ObstacleWorkshop)`, removed `OnExit`. `PreviewObstacle` entities manually despawned on exit. Async glTF node list population; placeholder cube if no glb match.
- **Drone pattern**: Async poll loading — `DroneGltfHandle` loaded `OnEnter(Race)`, `DroneAssets` extracted in Update polling, `spawn_drones` waits for both `DroneAssets` + `CourseData`. 12 drones with `DespawnOnExit(AppState::Race)`. Physics: 11-system `.chain()` in FixedUpdate (thrust-through-body model). Each drone gets a unique spline via `generate_drone_race_path()`. Per-drone variation via `DroneConfig`. `RaceSeed` resource randomizes outcomes between races. Resources cleaned up `OnExit(Race)`.
- **Course obstacle cleanup**: `spawn_obstacle()` adds `DespawnOnExit(AppState::Race)`. `CourseSpawned` marker removed `OnExit(Race)`.
- **Drone lifecycle**: `DronePhase` tracks state: `Idle → Racing → Returning → Idle` (or `Racing → Crashed` on DNF). Crashed drones: hidden, zero velocity, skipped by physics/AI. `ReturnPath` component holds return spline. `drones_are_active()` run condition keeps AI alive during Racing + Returning.
- **Race leaderboard**: `LeaderboardRoot` panel (top-left, `DespawnOnExit`). 12 rows with `LbColorBar`/`LbNameText`/`LbTimeText`. Updated from `RaceProgress::standings()` each frame. `DRONE_NAMES`/`DRONE_COLORS` in `drone/spawning.rs`.
- **Race validation**: `RacePhase`: `WaitingToStart → Countdown → Racing → Finished`. Race logic: 5-system `.chain()` in Update. Gate detection uses plane-crossing (line segment vs gate plane, directional, tunneling-proof). `miss_detection` is a safety net for drones that skip gates. `GatePlanes` resource cached at race start. Gate 0 is both start and finish (full lap).
...
- **Course props**: `PropInstance` in `CourseData.props` with `PropKind` (ConfettiEmitter/ShellBurstEmitter), optional `color_override`. `#[serde(default)]` for backward compat. Editor uses tabbed UI (`EditorTab::Obstacles`/`Props`), `PlacedProp` component, `PlacedFilter` type alias for shared queries.
...
- **Camera switching**: `CameraState` holds `CameraMode` (Chase/Fpv/Spectator). Keys: 1=Chase, 2=Spectator, 3=FPV (cycles target). Each mode has its own update system gated by `camera_mode_is()`. Chase follows leader with pack blending. FPV is stabilized close-follow. Spectator is RTS orbit (middle-mouse, scroll, WASD). `CameraHudRoot` shows mode/hints.
